...
export default function ProductDetailsPanel({
    ...
    const [activeTab, setActiveTab] = useState<'details' | 'summaries'>('details');
    ...
    return (
        <>
            ...
            {/* Modal Slide-over */}
            <div className="fixed right-0 top-0 bottom-0 w-full sm:w-[480px] lg:w-[540px] bg-white z-[100] flex flex-col shadow-2xl animate-slide-in-right">
                {/* Header Fijo */}
                <div className="flex-shrink-0 px-6 py-4 border-b border-slate-100 bg-gradient-to-r from-slate-50 to-white">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h2 className="text-lg font-black text-slate-900 tracking-tight">Detalles {isCard ? 'de Tarjeta' : 'del Producto'}</h2>
                                <p className="text-xs text-slate-500 font-medium">{institutionName}</p>
                            </div>
                        </div>
                        <button
                            onClick={onClose}
                            ...
                            aria-label="Cerrar"
                        >
                            <span className="material-symbols-outlined text-slate-400 hover:text-slate-900 text-[20px]">close</span>
                        </button>
                    </div>
                </div>
                ...
                <div className="flex-1 overflow-y-auto">
                    <div className="px-6 py-6 space-y-6">
                        {/* Tabs para tarjetas de crédito */}
                        {isCredit && (
                            <div className="flex gap-2 p-1 bg-slate-100 rounded-xl">
                                <button
                                    onClick={() => setActiveTab('details')}
                                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'details'
                                ...
                                <button
                                    ...
                                    className={`flex-1 px-4 py-2.5 rounded-lg text-sm font-bold transition-all ${activeTab === 'summaries'
                                        ? 'bg-white text-slate-900 shadow-sm'
                                        : 'text-slate-500 hover:text-slate-900'
                                        }`}
                                >
                        ...
                        {/* Summaries Panel (solo para tarjetas de crédito) */}
                        {isCredit && activeTab === 'summaries' && (
                            <SummaryPanel
                        ...
                        {/* Details Content (show for all or when details tab is active) */}
                        {(!isCredit || activeTab === 'details') && (
                            <>
                                {/* Balance Actual - Destacado (No mostrar para tarjetas de crédito) */}
                                {!isCredit && (
                                    <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-100 shadow-sm">
                                        <div className="flex items-center gap-2 mb-2">
                                            <span className="material-symbols-outlined text-blue-600 text-[24px]">account_balance_wallet</span>
                                            <p className="text-sm font-bold text-slate-600">Saldo Actual</p>
                                        </div>
                                        <p className={`text-4xl font-black tracking-tight ${product.balance >= 0 ? 'text-emerald-600' : 'text-rose-600'}`}>
